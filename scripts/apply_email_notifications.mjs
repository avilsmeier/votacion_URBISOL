...
insertAfter(
  'notification helpers after audit function',
...
async function logNotification({ election_id = null, registration_id = null, admin_user_id = null, template, recipient, status, error = null, meta_json = {} }) {
  try {
    ...
    // No dejamos que un problema de logging tumbe el flujo principal.
...
replaceIfFound(
  'approved token email title and log',
...
const sealedRoute = `app.post("/admin/notifications/sealed", requireAdmin, async (req, res) => {
...
if (!s.includes('app.post("/admin/notifications/sealed"')) {
  const marker = 'app.get("/admin/verify", requireViewerOrAdmin, async (req, res) => {';
  if (s.includes(marker)) {
    s = s.replace(marker, sealedRoute + "\n\n" + marker);
    changed = true;
  } else {
    console.warn('[WARN] No encontre marker para ruta sealed notifications');
  }
}

// Agrega botón al dashboard si el archivo existe.
...
if (fs.existsSync(dashboardFile)) {
  ...
  if (!d.includes('/admin/notifications/sealed')) {
    ...
    const to = `<a href="/admin/verify"><button>Verificar Integridad</button></a>
