...
patch("home validate link", txt => {
  if (txt.includes('app.locals.showVoteVerification')) return txt;
  return txt.replace(
    'const app = express();',
    'const app = express();\napp.locals.showVoteVerification = true;'
  );
});
...
async function lookupVoteReceipt({ receiptCode, identity }) {
  const receiptHash = sha256Hex(String(receiptCode || "").trim());
  const ident = String(identity || "").trim().toLowerCase();
  if (!receiptHash || !ident) return null;
...
const referendumRoute = `app.post("/votar/:token/referendum", async (req, res) => {
